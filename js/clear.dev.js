...
var templates = {
	...
	newList: function (pos) {
		...
		+ '<div class="inner">'
		+ '<div class="name">'        
        + '<input type="text" value="' + (pos == 'top' ? 'Pull to create item' : 'Tap me') + '"/></div>'
	...
	newTodo: function (pos) {
		...
		+ '<div class="inner">'
		+ '<div class="name">'
        + '<div class="custom-select"><select name="nerd-meta-key" id="nerd-meta-key">'
        + '    <option value="">Pick from me</option>'
	...
	render: function (list, index) {
		...
		.bind('swipeLeft', function(e){ //DELETE
			if (!window.globalDrag && !window.editing && !window.draggingDown) {
                ...
                $.ajax({
                    url         : '/remove-from-nerd',  
                    contentType : 'text/json',
                    dataType    : 'json',
                    data        : $('#listview').find('a.name').attr('_id')+'&'+$('#listview').find('a.name').attr('_meta'),
                    success     : function() {
                        todo.destroy();
                    }
                }); 
			...
			if (!window.editing && !window.inAction && !window.globalDragging && !window.draggingDown) {
				...
				$(this).html($('<input type="text" value="' + oname + '"/>').blur(function(){
					...
					if (!name) {
                        ...
                        var id = $('#listview').find('a.name').attr('_id');
                        var nerd = $('#listview').find('a.name').attr('_name');
                        var value = name;
                        console.log($('#listview').find('input').parent());
                        var key = ($('#listview').find('select').val() != '' ? $('#listview').find('select').val() : $('#listview').find('input').attr('_meta') );
	...
	renderSelf: function (home, index) {
		...
		.bind('swipeLeft', function(e){
			if (!window.globalDrag && !window.editing && !window.draggingDown) {
				var l = list.count();
				if (l != 0) {
					if (confirm('This employee contains ' + l + ' items. Are you sure you want to delete it?')) {
						list.destroy();
					} else {
						setTimeout(function(){
							list.home.reset();
...
$(function(){
	...
	var app = new Home([
		new List('Adam Argyle', [ // $.os.ios ? 'Hello':'Adam Argyle'
			new Todo('Seattle, WA'),
			new Todo('98116'),
			new Todo('Front End Developer'),
			new Todo('CSS, javascript, HTML'),
			new Todo('Linked In'),
			new Todo('M - F : 9 - 5')
		]),
		new List('Aaron Johnson', [
			new Todo('West Seattle, WA'),
			new Todo('CSS3'),
			new Todo('and Zepto.js')
		]),
		new List('Cody Cushing', [
			new Todo('@youyuxi'),
			new Todo('By the way'),
